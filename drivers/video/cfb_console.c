...
void draw_pixel(int x, int y,unsigned int color)
{
	unsigned char *dest = (unsigned char *) video_fb_address +
			((y * VIDEO_VISIBLE_COLS * VIDEO_PIXEL_SIZE) + x * VIDEO_PIXEL_SIZE);

	//if(x > VIDEO_VISIBLE_ROWS - 1 || x < 0 || y > VIDEO_VISIBLE_COLS - 1 || y < 0)
		//return;
...
void draw_bar(int x1, int y1,int x2,int y2,unsigned int color)
{
	int i,j;
	for(j = y1;j<=y2;j++)
		for(i = x1; i<=x2;i++)
...
static int video_init(void)
{
	...
	//keep the process bar in the center of the screen (402x32 is the bar size)
	x2 = (VIDEO_VISIBLE_COLS - 402)/2;y2 = (VIDEO_VISIBLE_ROWS - 32)/2;
	//keep the title at the top of the process bar 
	x1 = x2 + 90;y1 = y2 - 64;
	//keep the percence at the bottom of the process bar
	x3 = x2 + 90;y3 = y2 + 80;
