...
#endif
//Tony
#define I2C_GPIO_EEPROM_PROCESS

#include "../../../drivers/video/mxc_epdc_fb.h"
#include "../../../include/configs/mx6dl_sabresd_android.h"
...
//#define MAC_ADDRESS_BINARY	1
//#define MAC_ARRDREE_READ_SD
...
#define GPIO_I2C4_SCL		IMX_GPIO_NR(1, 28)
#define GPIO_I2C4_SDA		IMX_GPIO_NR(1, 29)
//gpio i2c speed (5) 100k, (50) 10k
...
static struct fb_videomode lvds_xga = {
	...
	/**
     * zengjf 2015-8-3 modify to 16bit 640x480 @ 40Hz
	 * can use this followimg argments
	 */
	"XGA", 40, 640, 480, 15385, 220, 40, 21, 7, 60, 10,

	/**
     * zengjf 2015-7-29 modify to 24bit 1366x768 @ 60Hz
     */
    //"XGA", 60, 1366, 768, 13825, 84, 36, 14, 3, 40, 5,
    //"XGA", 60, 1366, 768, 7825, 84, 36, 14, 3, 40, 5,
	//"XGA", 60, 1366, 768, 13255, 32, 112, 18, 18, 10, 2,

	/**
     * zengjf 2015-7-24 modify to 24bit 1280x800 @ 60Hz
     */
    //"XGA", 60, 1280, 800, 13966, 110, 53, 8, 8, 7, 7,

	/**
     * zengjf 2015-7-24 modify to 18bit 1024x800 @ 60Hz
     */
    //"XGA", 60, 1024, 768, 17818, 84, 36, 14, 3, 40, 5,
	...
	*/
	//"XGA", 40, 800, 480, 25986, 220, 40, 21, 7, 60, 10,
	//"XGA", 40, 800, 480, 20986, 220, 40, 21, 7, 60, 200,
	//"XGA", 40, 640, 480, 19861, 504, 16, 11, 32, 10, 2, 
	//"XGA", 40, 640, 480, 19861, 304, 16, 11, 32, 10, 2, 
	//"XGA", 40, 640, 480, 19861, 254, 16, 11, 32, 10, 12, 
...
#ifdef I2C_GPIO_EEPROM_PROCESS
static int i2c_gpio_sda_get(void)
{
	return gpio_get_value(GPIO_I2C4_SDA);
}

static void i2c_gpio_sda_input(void)
{
	gpio_direction_input(GPIO_I2C4_SDA);
}
...
static void i2c_gpio_sda_set(int bit)
{
	if (bit) {
		...
		gpio_direction_output(GPIO_I2C4_SDA,bit);
	}
}
...
{
	gpio_direction_output(GPIO_I2C4_SCL,bit);
}
...
int i2c_gpio_write_eeprom(uchar chip, uint addr, int alen, uchar *buffer, int len)
{
...
//		chip, addr, alen, buffer, len);
	...
	i2c_gpio_send_start();
	if(i2c_gpio_write_byte(chip << 1)) {	/* write cycle */
		i2c_gpio_send_stop();
...
int i2c_gpio_read_eeprom(uchar chip, uint addr, int alen, uchar *buffer, int len)
{
	int shift;
	i2c_gpio_send_start();
	if(alen > 0) 
	{
		if(i2c_gpio_write_byte(chip << 1)) 
		{ /* write cycle */
			i2c_gpio_send_stop();
			puts("i2c_read, no \n");
			return(1);
	 ...
	 */
	i2c_gpio_write_byte((chip << 1) | 1);	/* read cycle */
	while(len-- > 0) {
...
static int i2c_gpio_eeprom_init(uchar chip)
{
	int ret;
	
	 i2c_gpio_send_start();
	 ret = i2c_gpio_write_byte((chip << 1) | 0);
	 i2c_gpio_send_stop();
	return ret;
}
...
#if defined CONFIG_MX6Q
		/* GPIO to I2C4 SCL*/
		mxc_iomux_v3_setup_pad(MX6Q_PAD_ENET_TX_EN__GPIO_1_28);
		/* GPIO to I2C4 SDA*/
		mxc_iomux_v3_setup_pad(MX6Q_PAD_ENET_TXD1__GPIO_1_29);
#elif defined CONFIG_MX6DL
		/* GPIO to I2C4  SCL*/
		mxc_iomux_v3_setup_pad(MX6DL_PAD_ENET_TX_EN__GPIO_1_28);
		/* GPIO to I2C4 SDA*/
	...
	if (mmc) 
	{
		...
		if (dev_desc!=NULL) 
		{
			if (fat_register_device(dev_desc,1)==0) 
			{
				int check=0;
				offset = &tbuffer;
				size = file_fat_read ("mac.txt", (unsigned char *) offset, sizeof(tbuffer));
				if(size==17) 
				{
#ifdef MAC_ADDRESS_BINARY
					//70:B3:D5:10:6F:56 
					//70:B3:D5:10:6F:57
					puts("mac.txt: ");
					for(i=0;i<size;i++)
					{
						sprintf(tmp," %02x",tbuffer[i]);
						puts(tmp);
					}
					puts("\n");
                    //读取mac地址
					buffer[0]=datatransfer(tbuffer[0],	tbuffer[1]);
					buffer[1]=datatransfer(tbuffer[3],	tbuffer[4]);
					buffer[2]=datatransfer(tbuffer[6],	tbuffer[7]);
					buffer[3]=datatransfer(tbuffer[9], 	tbuffer[10]);
					buffer[4]=datatransfer(tbuffer[12],	tbuffer[13]);
					buffer[5]=datatransfer(tbuffer[15],	tbuffer[16]);
                    // 写eeprom, 分成6字节写
					check=i2c_gpio_write_eeprom(0x50, 0x0, 1, buffer, 6);
					if(check>0)
					{
						sprintf(tmp," Error count:%d\n",check);
						puts(tmp);
					}
					puts("mac : ");
					for(i=0;i<6;i++)
					...
					puts("\n");
//					puts("i2c_gpio_write_eeprom test \n");
#else
                    // 写mac，分17字节写入
					...
					puts(buffer);
					puts("\n");
					check=i2c_gpio_write_eeprom(0x50, 0x0, 1, buffer, 8);
					if(check>0)
					{
						sprintf(tmp," Error count:%d\n",check);
						puts(tmp);
					}
					check=i2c_gpio_write_eeprom(0x50, 0x8, 1, buffer+8, 8);
					if(check>0)
					{
						sprintf(tmp," Error count:%d\n",check);
						puts(tmp);
					}
					check=i2c_gpio_write_eeprom(0x50, 0x10, 1, buffer+16, 1);
					if(check>0)
					{
						sprintf(tmp," Error count:%d\n",check);
						puts(tmp);
...
#ifdef I2C_GPIO_EEPROM_PROCESS		//eeprom read process
	{
		uchar buffer[128]={0};
		uint k;
		char tmp[128]={0};
		...
		for(k=0x0;k<=0xff;k++)
		{
			i2c_gpio_read_eeprom(0x50, k, 1, buffer, 1);
			sprintf(tmp,"%d = %02x\r\n",k,buffer[0]);
			puts(tmp);
		}
		#else
        // 读取mac地址
		i2c_gpio_read_eeprom(0x50, 0x0, 1, buffer, 6);
//		puts("Read EEPROM:");
//		for(k=0;k<6;k++)
//		{
//			sprintf(tmp," %02x",buffer[k]);
//			puts(tmp);
//		}
//		puts("\n");
		sprintf(tmp,"%02x:%02x:%02x:%02x:%02x:%02x",buffer[0],buffer[1],buffer[2],buffer[3],buffer[4],buffer[5]);
        // 设置环境变量
		setenv("ethaddr",tmp);
			#if 0
		{
			unsigned int value=0x0;
			value=buffer[5]+(buffer[4]<<8)+(buffer[3]<<16)+(buffer[2]<<24);
			writel(value, MX6_IO_ADDRESS(OCOTP_BASE_ADDR) + HW_OCOTP_MACn(0));
			sprintf(tmp,"HW_OCOTP_MACn(0)%p:%p\n",(MX6_IO_ADDRESS(OCOTP_BASE_ADDR) + HW_OCOTP_MACn(0)),value);
			puts(tmp);
			value=buffer[1]+(buffer[0]<<8);
			writel(value, MX6_IO_ADDRESS(OCOTP_BASE_ADDR) + HW_OCOTP_MACn(1));
			sprintf(tmp,"HW_OCOTP_MACn(1)%p:%p\n",(MX6_IO_ADDRESS(OCOTP_BASE_ADDR) + HW_OCOTP_MACn(1)),value);
			puts(tmp);
		}
			#endif
		#endif
	#else
		i2c_gpio_read_eeprom(0x50, 0x0, 1, buffer, 17);
//		puts("Read EEPROM:\n");
//		for(k=0;k<17;k++)
//		{
//			sprintf(tmp," %02x",buffer[k]);
...
#else
//	setenv("ethaddr","70:B3:D5:10:6F:56");
#endif
}
#endif
