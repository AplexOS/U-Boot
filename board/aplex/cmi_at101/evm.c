...
int board_evm_init(void)
{
	...
	if (board_id == IA_BOARD)
		...
		gd->bd->bi_arch_number = MACH_APLEX_CMI_AT101;
...
int board_init(void)
{
	...
	configure_evm_pin_mux(board_id, header.version, profile, daughter_board_connected);
		...
		/*set gpio0_7 gpio0_12 gpio0_22 gpio0_23 output to control backlight ,mode*/
		__raw_writel(~((1<<7) | (1<<12) | (1<<22) |(1<<23)), 0x44E07134);
		/*set gpio0_7 12 19 23 low to close backlight, mode*/
		__raw_writel((1<<7) |(1<<19) | (1<<23), 0x44E07190);
		//__raw_writel((1<<12) |(1<<19) | (1<<23), 0x44E07190);

		/*set gpio0_22 high to open buzzer*/
		//__raw_writel((1<<7) | (1<<22), 0x44E07194);
		__raw_writel((1<<12) | (1<<22), 0x44E07194);

# if 0
	/*gpio about led buzzer backlight*/
#if defined(CONFIG_OK335XD)
		/*set gpio0_7 output to control backlight*/
		__raw_writel(~0x80, 0x44E07134);

		/*set gpio1_16 ~ gpio1_19 output to control led*/
		__raw_writel(~0xF0000,0x4804C134); 

		/*set gpio0_7 low to close backlight*/
		__raw_writel(1<<7, 0x44E07190);
#elif defined(CONFIG_OK335XS)
		/*set gpio3_17 and gpio3_16 output to control backlight and led*/
		__raw_writel(~0x30000, 0x481AE134);

		/*set gpio0_7 output to control buzzer*/
		__raw_writel(~0x80, 0x44E07134);

		/*set gpio3_17 low to close backlight*/
		__raw_writel(1<<17, 0x481AE190);
		
		/*set gpio0_7 high to open buzzer*/
		__raw_writel(1<<7, 0x44E07194);
		udelay(10000);
		/*set gpio0_7 low to close buzzer*/
		__raw_writel(1<<7,0x44E07190);
		while(c--)udelay(20000);
#elif defined(CONFIG_OK335XS2)
		/*set gpio3_17 output to control backlight*/
		__raw_writel(~0x20000, 0x481AE134);

		/*set gpio1_12 ~ gpio1_15 output to control led*/
		__raw_writel(~0xF000,0x4804C134);
...
static struct cpsw_slave_data cpsw_slaves[] = {
	{
		...
		/*ok335xs2 use regmii2 but i want the port to be recognized as eth0*/
		.slave_reg_ofs	= 0x308,	
...
#define AR8051_RGMII_TX_CLK_DLY		0x100
	...
	miiphy_write(devname, CONFIG_PHY_ADDR, AR8051_PHY_DEBUG_ADDR_REG,
			AR8051_DEBUG_RGMII_CLK_DLY_REG);
	miiphy_write(devname, CONFIG_PHY_ADDR, AR8051_PHY_DEBUG_DATA_REG,
